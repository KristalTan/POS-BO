...
const checkoutSchema = yup.object().shape({
    ...
    tax_code1: yup.number().required("Tax Code 1 is required"),
    tax_code2: yup.number().required("Tax Code 2 is required"),
    is_in_use: yup.number().required("Status is required"),
    amt_include_tax1: yup.number().nullable(),
    amt_include_tax2: yup.number().nullable(),
});

const Edit_Prod_Item = () => {
    ...
    const initialValues = {
        ...
        product_tag: response?.product_tag || "",
        // product_img_path: response?.product_img_path || "",
        product_img_path: "",
        supplier_id: response?.supplier_id || 0,
        pricing_type_id: response?.pricing_type_id || 0,
        cost: response?.cost || 0.00,
        sell_price: response?.sell_price || 0.00,
        tax_code1: response?.tax_code1 || 0,
        amt_include_tax1: response?.amt_include_tax1 || "0.00",
        tax_code2: response?.tax_code2 || 0,
        amt_include_tax2: response?.amt_include_tax2 || "0.00",
        calc_tax2_after_tax1: response?.calc_tax2_after_tax1 || 0.0,
        is_in_use:response?.is_in_use || 1,
        display_seq: response?.display_seq || "",
        is_enable_kitchen_printer: response?.is_enable_kitchen_printer || 0,
        is_allow_modifier: response?.is_allow_modifier || 1,
        is_enable_track_stock: response?.is_enable_track_stock || 1,
        is_popular_item: response?.is_popular_item || 1,
        meal_period: response?.meal_period || 0,
    };
    ...
    return (
        <Box m="20px">
            <Box display="flex" alignItems="center" mb="2px">
                ...
                <Box ml={1}>
                    <Header title="Edit Product Item" />
                </Box>
            </Box>
            <Box
                border="0.1px solid"
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            >
                                ...
                                <TextField
                                    fullWidth
                                    variant="filled"
                                    value={values.amt_include_tax1.toFixed(2)}
                                    onChange={handleChange}
                                ...
                                <TextField
                                    fullWidth
                                    variant="filled"
                                    value={values.amt_include_tax2.toFixed(2)}
                                    onChange={handleChange}
